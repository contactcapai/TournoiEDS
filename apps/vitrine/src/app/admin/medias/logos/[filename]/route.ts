...
/**
 * Service des logos POUR LE BACK-OFFICE — `/admin/medias/logos/<nom>` (Story 6.5).
 *
 * Même montage que `/admin/medias/[filename]` (Story 6.4), pour la même raison : la route
 * publique filtre sur `is_published` et rend **404** sinon, volontairement. Un aperçu naïf
 * afficherait donc des **cadres cassés exactement sur les logos qu'on veut regarder avant de
 * publier** — c'est-à-dire au seul moment où l'écran doit dire la vérité.
 *
 ...
 *     enfant, et le `307` portait le tableau de bord entier dans sa charge RSC ;
 ...
 * (`lib/logos.ts`). Mesuré au gate visuel de la 6.4 : sans cela, **aucune vignette ne
 * s'affiche**.
 */
...
export async function GET(
  ...
  // 🔴 PREMIÈRE INSTRUCTION, AVANT TOUTE LECTURE. Cette route sert les logos de partenaires
  // NON PUBLIÉS : sans cette ligne, elle serait une fuite de données, pas une commodité.
  ...
  try {
    ligne = await db.query.partner.findFirst({
      ...
      where: (table, { eq }) => eq(table.logo, cheminLogo(filename)),
    });
  } catch {
    return introuvable();
  }
